...
function html_onload() {
  ...
  let uploading = false; // Replace ref with a boolean variable
  ...
  async function DownloadFile(fileName, totalChunks, fileId) {
    ...
    try {
      ...
      for (let i = 1; i <= totalChunks; i++) {
        const chunk = await DownloadChunkWithRetry(fileName, i, fileId);
  ...
  async function DownloadChunkWithRetry(fileName, currentChunk, fileId) {
    ...
    while (attempts < maxAttempts) {
      try {
        ...
        console.log('Chunk download error:', error.message);
        attempts++;
    ...
    if (attempts === maxAttempts) {
      console.error('Chunk download failed:', currentChunk);
  ...
  async function DownloadChunk(fileName, chunk, fileId) {
    try {
      const response = await fetch(`/api/download/${fileName}?chunk=${chunk}&file=${fileId}&db=fileDocumentDicom`, {
        method: 'GET',
      ...
      throw new Error('Chunk download failed: ' + error);
    }
  }
  ...
  getByid("myfile").onclick =async  function  () {
    try{

      var url = window.location.href;

      var regex = /\/doctor\/file\/dicom\/([^\/]+)/;
      var match = url.match(regex)[1];
      ...
      let buffer = await fetch(`${baseUrl}/api/download/fileInfo/${match}`,
      {
        method: 'GET',
        credentials: "include",
      });
      ...
      buffer = await buffer.json();
      
      
      for(var i = 0; i<buffer.length; i++){

        
        const base64Data = buffer[i].fileData;
